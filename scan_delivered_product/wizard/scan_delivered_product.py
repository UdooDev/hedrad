...
class WizardProductSerial(models.TransientModel):
	...
	def onchange_serial(self):
		if self.serial:
			...
			lot = self.env['stock.production.lot'].search([('name', '=', self.serial.strip())], limit=1)
			...
			if move_line:
				...
				move_lines = self.picking_id.move_line_ids.filtered(lambda r: not r.lot_id and r.product_id.id == lot.product_id.id)
				if move_lines:
					move_lines[0].write({
						'qty_done': 1,
						'lot_id': lot.id,
